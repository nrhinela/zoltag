#!/usr/bin/env python3
"""Backfill permatags.asset_id and machine_tags.asset_id from image_metadata.asset_id.

Uses set-based SQL UPDATEs for speed.

Usage:
  python scripts/backfill_tag_tables_asset_id.py --tenant-id bcg --batch-size 5000
  python scripts/backfill_tag_tables_asset_id.py --table permatags --dry-run
  python scripts/backfill_tag_tables_asset_id.py --update-mismatched
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backfill tag table asset_id fields.")
    parser.add_argument("--tenant-id", help="Only process rows for this tenant id.")
    parser.add_argument(
        "--table",
        choices=["permatags", "machine_tags", "all"],
        default="all",
        help="Which table to process.",
    )
    parser.add_argument("--limit", type=int, help="Max rows to process per selected table.")
...
def _collect_stats(
    ...
    *,
    ...
    tenant_clause = "AND t.tenant_id = :tenant_id" if tenant_id else ""
    ...
    stats.missing_image = _scalar(
        ...
        SELECT count(*)
        FROM {table_name} t
        LEFT JOIN image_metadata im ON im.id = t.image_id
        WHERE im.id IS NULL
        {tenant_clause}
        """,
        params,
    ...
    mismatched = _scalar(
        ...
        WHERE im.asset_id IS NOT NULL
          ...
          AND t.asset_id IS DISTINCT FROM im.asset_id
        {tenant_clause}
        """,
        params,
    )
    ...
    eligible_clause = "t.asset_id IS DISTINCT FROM im.asset_id" if update_mismatched else "t.asset_id IS NULL"
...
def _run_backfill_for_table(
    ...
    *,
    ...
    while remaining > 0:
        ...
        last_id_clause = "AND t.id > :last_id" if last_id is not None else ""

        sql = f"""
        WITH candidate AS (
            SELECT t.id, im.asset_id
            FROM {table_name} t
            JOIN image_metadata im ON im.id = t.image_id
            WHERE im.asset_id IS NOT NULL
              AND {eligible_clause}
...
def _print_progress(table_name: str, stats: BackfillStats) -> None:
    print(
        f"[{table_name}] Processed={stats.processed} Updated={stats.updated} "
        f"AlreadyCorrect={stats.already_correct} MissingImage={stats.missing_image} "
        f"MissingImageAsset={stats.missing_image_asset} MismatchedSkipped={stats.mismatched_skipped}"
    )
...
def main() -> None:
    ...
    try:
        targets = []
        if args.table in ("all", "permatags"):
            targets.append("permatags")
        if args.table in ("all", "machine_tags"):
            targets.append("machine_tags")

        for table_name in targets:
            stats = _run_backfill_for_table(
                session,
                table_name=table_name,
            ...
            _print_progress(table_name, stats)
    finally:
        session.close()
